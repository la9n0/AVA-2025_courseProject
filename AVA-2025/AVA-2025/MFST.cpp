...
namespace MFST
{
	...
	Mfst::RC_STEP Mfst::step(const Log::LOG& log) //��������� ��� ��������
	{
		...
		if (lenta_position < lenta_size)
		{
			...
			else { MFST_TRACE4(log, "TS_NOK/NS_NORULECH�IN") rc = reststate(log) ? TS_NOK : NS_NORULECHAIN; }; //��� ������ ��� �������
		}
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case LENTA_END:         MFST_TRACE4(log, "------>LENTA_END")
			* log.stream << "-------------------------------------------------------------------------------------" << std::endl;
			oss << 0 << ":Длина ленты " << lenta_size << ", синтаксический разбор завершен без ошибок ";
			if (log.stream == &std::cout)
				Log::printConsoleUtf8(oss.str().c_str());
			else
				*log.stream << std::setw(4) << std::left << oss.str() << std::endl;
			rc = true;
			break;
	...
	char* Mfst::getDiagnosis(short n, char* buf)//����� ������
	{
		...
		{
			strncpy_s(buf, MFST_DIAGN_MAXSIZE, "", _TRUNCATE);
		}
		return rc;
	};
