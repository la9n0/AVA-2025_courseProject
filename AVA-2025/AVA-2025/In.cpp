...
namespace In
{
	IN getin(wchar_t infile[], std::ostream* stream)			// ������ � ��������� ������� �����
	{
		...
		if (!instream.is_open())
			throw ERROR_THROW(102);								//"��������� ������: ������ ��� �������� ����� � �������� �����(-in)"
		IN in;
		int pos = 0;
		bool isLiteral = false;
		// Пропускаем BOM UTF-8 (0xEF 0xBB 0xBF) если есть
		unsigned char b1 = static_cast<unsigned char>(instream.peek());
		if (b1 == 0xEF)
		{
			instream.get();
			unsigned char b2 = static_cast<unsigned char>(instream.peek());
			if (b2 == 0xBB)
			{
				instream.get();
				unsigned char b3 = static_cast<unsigned char>(instream.peek());
				if (b3 == 0xBF)
				{
					instream.get(); // успешно пропустили BOM
				}
				else
				{
					// Вернём один байт назад, чтобы не потерять данные
					instream.unget();
				}
			}
			else
		...
		while (true)
		{
			...
			if (instream.eof())
				break;
			// Игнорируем CR (Windows \r\n), чтобы не попадал в лексемы и не ронял автоматы
			if (uch == '\r')
				continue;
			if (in.code[uch] == IN::Q)   // ������� 
				isLiteral = !isLiteral;
