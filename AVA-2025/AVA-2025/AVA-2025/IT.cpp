...
namespace IT {
	...
	void AddFunctionStaticLib(IdTable& idtable) {
		Entry powFunction(0, (char*)"Pow", IDDATATYPE(1), IDTYPE(5)); // INT
		Entry sumFunction(1, (char*)"Sum", IDDATATYPE(1), IDTYPE(5)); // INT
		Entry strcmpFunction(2, (char*)"StrCmp", IDDATATYPE(1), IDTYPE(5)); // INT ( )
	...
	void Delete(IdTable& idtable) {
		// IdTable использует std::vector, который сам управляет памятью
