...
namespace PolishNotation {
	...
	void FixLT(LT::LexTable& lextable, const std::string& str, int length, int pos, std::vector<int>& ids, Log::LOG log) {
		...
		Log::WriteLine(log, (char*)"Преобразование в польскую запись\n", (char*)str.c_str(), "\n", "");
	...
	bool PolishNotation(int& pos, LT::LexTable& lextable, IT::IdTable& idtable, Log::LOG log) {
		...
		for (int i = pos; i < lextable.size; i++, iterator++) {
			...
			case LEX_INVERSION:
			{
				// Унарные операции добавляются в польскую запись
				PolishString += lexem;
				operators_count++;
				break;
			}
			case LEX_COMMA:
			{
				// Выталкиваем все операторы на стек до открывающей скобки.
				while (!stack.empty()) {
