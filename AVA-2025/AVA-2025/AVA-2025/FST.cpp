...
namespace FST {
	...
	bool execute(FST& fst) {
		...
		unsigned short lstring = (unsigned short)fst.string.size();
		bool rc = true;
		...
		for (int i = 0; i < lexems.size(); i++) {
			for (int j = 0; j < FSTarray.size(); j++) {
				...
				if (execute(*FSTarray[j].fst)) {
					...
					case LEX_SYMBOL:
					{
						if (FSTarray[j].iddatatype == IT::SYMBOL) {
					...
					for (size_t g = 0; g < lexems[i].word.size(); g++) {
					...
					case LEX_LITERAL:
					{
						...
						if (lexems[i].word[0] == '\"') {
							iddatatype = IT::SYMBOL; //  
						ITObj.value.vtext->len = (int)lexems[i].word.size();
						for (size_t g = 0; g < lexems[i].word.size(); g++) {
						...
						ITObj.value.vtext->str[lexems[i].word.size()] = '\0';
							...
							iddatatype = IT::INT;
							ITObj.value.vint = (short)VectorToInt(lexems[i].word);
						}
					...
					if (LTObj.lexema == LEX_INCREMENT || LTObj.lexema == LEX_DECREMENT || LTObj.lexema == LEX_INVERSION || LTObj.lexema == LEX_LEFTBRACE || LTObj.lexema == LEX_BRACELET || LTObj.lexema == LEX_LEFTTHESIS || LTObj.lexema == LEX_RIGHTTHESIS) {
	...
	void Analyze(In::IN in, LT::LexTable& lextable, IT::IdTable& idtable) {
		...
		std::vector<FSTAssigned> FSTarray = {
			...
			FSTAssigned(&lex_function, (IT::IDDATATYPE)0, LEX_FUNCTION),        // 2
			FSTAssigned(&lex_declare, (IT::IDDATATYPE)0, LEX_DECLARE),          // 3
			FSTAssigned(&lex_return, (IT::IDDATATYPE)0, LEX_RETURN),			// 4
			FSTAssigned(&lex_display, (IT::IDDATATYPE)0, LEX_DISPLAY),			// 5
			FSTAssigned(&lex_main, (IT::IDDATATYPE)0, LEX_MAIN),				// 6

			FSTAssigned(&lex_semicolon, (IT::IDDATATYPE)0, LEX_SEMICOLON),		// 7
			FSTAssigned(&lex_comma, (IT::IDDATATYPE)0, LEX_COMMA),				// 8
			FSTAssigned(&lex_leftbrace, (IT::IDDATATYPE)0, LEX_LEFTBRACE),		// 9
			FSTAssigned(&lex_bracelet, (IT::IDDATATYPE)0, LEX_BRACELET),		// 10
			FSTAssigned(&lex_leftthesis, (IT::IDDATATYPE)0, LEX_LEFTTHESIS),	// 11
			FSTAssigned(&lex_rightthesis, (IT::IDDATATYPE)0, LEX_RIGHTTHESIS),  // 12

			FSTAssigned(&lex_assigment, (IT::IDDATATYPE)0, LEX_ASSIGNMENT),	    // 13

			FSTAssigned(&lex_for, (IT::IDDATATYPE)0, LEX_FOR),			// 14

			FSTAssigned(&lex_increment, (IT::IDDATATYPE)0, LEX_INCREMENT),				// 15
			FSTAssigned(&lex_decrement, (IT::IDDATATYPE)0, LEX_DECREMENT),				// 16
			FSTAssigned(&lex_inversion, (IT::IDDATATYPE)0, LEX_INVERSION),				// 17

			FSTAssigned(&lex_id, (IT::IDDATATYPE)0, LEX_ID),					// 18
			FSTAssigned(&lex_textLiteral, IT::SYMBOL, LEX_LITERAL),				// 19 ( )
			FSTAssigned(&lex_symbolLiteral, IT::SYMBOL, LEX_LITERAL),			// 20
			FSTAssigned(&lex_byteLiteral, IT::INT, LEX_LITERAL),				// 21
			FSTAssigned(&lex_booleanLiteral, IT::INT, LEX_LITERAL)				// 22 ( )
		};
